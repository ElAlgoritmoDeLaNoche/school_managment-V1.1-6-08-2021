<?php
	...
	function validaIdToken($id, $token){
		...
		if($rows > 0) { //si el dato es mayor a 0
	...
	function login($usuario, $password)
	{
		...
		if($rows > 0) { //si el valor es mayor a 0
			
			if(isActivo($usuario)){ //si el usuario esta activo
				
				$stmt->bind_result($id, $id_tipo, $passwd);//parametro (s=string, variable)
	...
	function isActivo($usuario)
	{
		...
		$stmt->bind_param('ss', $usuario, $usuario); //parametro (s=string, variable)
		$stmt->execute();//se ejecuta la sentencia
		$stmt->bind_result($activacion); //visualizacion de la sentencia
...
//funcion llamada verificaTokenPass,
	function verificaTokenPass($user_id, $token){
		...
		$stmt->bind_param('is', $user_id, $token); //parametro (i=int, variable)
		$stmt->execute(); //se ejecuta la sentencia
		$stmt->store_result(); //se obtiene los resultados
		$num = $stmt->num_rows;  //guarda los datos
		
		if ($num > 0) //si el valor que tiene la variable num  es mayor a 1
		{
			$stmt->bind_result($activacion); //se obtiene los resultados
			$stmt->fetch(); //guarda los resultados
			if($activacion == 1) //si el valor que tiene la variable activacion es igual a 1
			{
				return true; //retorna true
			}
			else // de lo contrario
