<?php
  ...
  if(isset($_REQUEST['delete'])){
    ...
    $sql = "DELETE FROM blog WHERE id = $id"; //sentencia SQL
    mysqli_query($mysqli, $sql); //acceder a la base de datos y ejecutar la sentencia

    header("Location: feedback"); //redireccionamiento a feedback
    exit();
  }
            ...
            <div class="card-body">
              ...
              ?>
                <div class="bg-dark p-5 rounded-lg text-white text-center">
                  <h1><?php echo $titulo;?></h1> <!--imprime lo que tenga la variable titulo -->
                  <div class="d-flex mt-2 justify-content-center align-items-center">
                    <a href="edit.php?id=<?php echo $q['id']?>" class="btn btn-light btn-sm" name="edit">Editar</a> <!--link que redirecciona a edit.php -->
                    <form method="POST">
                      <input type="text" hidden value='<?php echo $q['id']?>' name="id"><!--imprime lo que tenga la variable q -->
                      <button class="btn btn-danger btn-sm ml-2" name="delete">Borrar</button>
                    </form>
                  </div>
                </div>
                  <p class="mt-5 border-left border-dark pl-3"><?php echo $descripcion;?></p><!--imprime lo que tenga la variable descripcion -->
                <?php } ?>
                  <a href="feedback" class="btn btn-outline-dark my-3">Regresar</a>
              </div>
          </div>
        </section>
      </div>
